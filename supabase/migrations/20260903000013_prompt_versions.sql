-- Prompt versions are experiments until adopted; only adopted versions feed venue_profiles and the explorer.
...
insert into prompt_versions (version, adopted, notes) values
('v1', false, 'invented good_for values (echoed prompt examples); superseded'),
('v2', true,  'grounding filter; vibe inferred from awards/press'),
('v3', true,  'no vibe inference from awards/press/history; under-extracts best_time and events'),
('v4', false, 'experiment: cue lists with example values -> echoed values, neutral over-applied; rejected'),
('v5', false, 'experiment: cues without example values, neutral restricted')
...
with ins as (
    ...
    where i.venue_id is not null and i.prompt_version in (select version from prompt_versions where adopted)
),
tags as (select venue_id, tag, count(*) as n from ins, unnest(vibe_tags) as tag group by 1, 2),
tag_agg as (
    select venue_id, jsonb_object_agg(tag, n order by n desc, tag) as vibe_tag_counts,
           array_agg(tag order by n desc, tag) as vibe_tags_ranked from tags group by 1
)
select v.id as venue_id, v.key, v.name, v.category, v.housenumber, v.street, v.zip, v.lat, v.lon,
       ...
       round(avg(case ins.sentiment when 'positive' then 1 when 'mixed' then 0 when 'neutral' then 0 when 'negative' then -1 end)::numeric, 2) as sentiment_score,
...
grant select on venue_profiles to anon, authenticated;
